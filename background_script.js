...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (changeInfo?.status === "complete") {
    ...
    if (tab.url.includes("booking/psgninput")) {
      console.log("Passenger Input started");
      chrome.tabs.sendMessage(tabDetails.id, getMsg("fillPassengerDetails"));
    }
    if (tab.url.includes("booking/reviewBooking")) {
      console.log("Review captcha page")
      chrome.tabs.sendMessage(tabDetails.id, getMsg("focusCaptchaInput"));
    }
    if (tab.url.includes("payment/bkgPaymentOptions")) {
      console.log("Payment selection page")
      chrome.tabs.sendMessage(tabDetails.id, getMsg("selectPaytmUPI"));
    }
  }
});
