...
async function delayUntil(targetHour, targetMinute) {
  console.log('Delay until => '+targetHour+':'+targetMinute)
  let now = new Date();
  let then = new Date(now);
  then.setHours(targetHour);
  then.setMinutes(targetMinute);
  then.setSeconds(0);
  ...
  let delay = then.getTime() - now.getTime();
...
function clickWithContent(query, text) {
  return new Promise(resolve => {
        let intervalId = setInterval(function() {
        let element = Array.from(document.querySelectorAll(query)).find(el => el.textContent.trim() === text);
        if (element) {
            element.click();
...
function selectPaytmUPI() {
  console.log('Selecting paytm upi')
  clickWithContent('span.col-pad', 'BHIM/ UPI/ USSD')
    .then(() => clickWithContent('span.col-pad', 'Pay using BHIM (Powered by PAYTM ) also accepts UPI'))
    .then(() => clickWithContent('#psgn-form button', 'Pay & Book'));
}
...
function loadJourneyDetails() {
  ...
  fromInputField.value = user_data["journey_details"]["from"]
    ? `${user_data["journey_details"]["from"]["value"]}`
    : "";
  ...
  destinationInputField.value = user_data["journey_details"]["destination"]
    ? `${user_data["journey_details"]["destination"]["value"]}`
    : "";
...
function delayAndSelectJourney() {
  ...
  if (
    ...
    (user_data["journey_details"]["quota"]["label"] === "PREMIUM TATKAL" &&
      ...
      (async function() {
        if(["1A", "2A", "3A", "CC", "EC", "3E"].includes(jclass.toUpperCase())) {
          await delayUntil(10, 0);
        } else {
          await delayUntil(11, 0);
        }
        selectJourney()
      })();
    }
...
function fillPassengerDetails() {
  ...
  // GSTIN details
  if (user_data.hasOwnProperty("gst_details") && user_data["gst_details"]["gstin-number"]) {
    addDelay(400);
